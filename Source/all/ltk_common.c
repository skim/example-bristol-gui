...
void ltk_combo_box_fill(GtkComboBox *combo_box, LValueList *values) {
	...
	if (l_value_list_get_type(values) == L_TYPE_STRING) {
		...
		for (i = 0; i < l_value_list_length(values); i++) {
			...
			gtk_list_store_set(store, &iter, 0, l_value_list_nth_display_name(values, i), 1, l_value_get_string(value), -1);
		}
		...
		g_warning("type of value list is currently unsupported");
	}
}
